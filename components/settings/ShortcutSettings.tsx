    ...
    return (
        <div className="space-y-2">
             <div className="flex items-center gap-2 text-zinc-400 mb-4">
                <Keyboard size={16} />
                <span className="text-sm font-bold uppercase">Keyboard Shortcuts</span>
             </div>
                 ...
                 {actions.map(action => (
                     <div key={action} className="flex items-center justify-between bg-zinc-950 p-2 rounded border border-zinc-800">
                         ...
                         <div className="flex items-center gap-2">
                             ...
                             <button onClick={() => clear(action)} className="text-zinc-600 hover:text-red-400 text-xs">Clear</button>
                         </div>
                     </div>
                 ))}
             </div>
             <p className="text-xs text-zinc-500 mt-4">Click on a box and press the desired key combination to bind.</p>
        </div>
    );
};
